...
数据仅统计 Profile 主体，排除进程创建、首次提示符渲染和终端绘制，未控制磁盘冷缓存，不应称为完整冷启动。旧审计的默认模式开启了更多功能，full 又跳过了超时的 vfox，因此不能据此宣传同配置提速倍数。测量方法见 `tests/Measure-ProfileStartup.ps1`，历史证据见《审计修复复核-2026-09-07.md》。
