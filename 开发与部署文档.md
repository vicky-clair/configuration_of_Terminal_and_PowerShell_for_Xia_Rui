...
### Q6: Scoop 安装字体报错 `Couldn't find manifest for 'JetBrainsMono-NF' from 'nerd-fonts' bucket`？
**解答**：网络波动可能导致 bucket 目录仅有空文件夹而缺失清单文件，或清单检索失败。
- **自动自愈**：
  1. 脚本自动检测 bucket 目录下是否存在 `.json` 清单，若损坏自动清除重拉；
  2. 若 Scoop 仍无法安装字体，脚本自动触发**第四梯队：CDN 极速直链下载**（通过 `gh-proxy.com` 下载官方 Nerd Font 压缩包），自动解压至系统字体目录并注册当前用户注册表字体项，全自动保底完成。
...
1. 请确保运行过本套件的字体安装（或前往 [JetBrainsMono Nerd Font](https://www.nerdfonts.com/font-downloads) 手动下载安装）；
